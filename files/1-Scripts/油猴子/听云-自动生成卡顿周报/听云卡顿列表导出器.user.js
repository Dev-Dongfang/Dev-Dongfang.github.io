...
//        targetNode.appendChild(alertView);
    ...
    let transformBuglyElementsToModel = function(elements) {
        ...
        for(var i = 0; i < elements.length; i++) {
            ...
            for(var j = 1; j <= blocklabels.length - 2; j++) { tags +="blockLabels[j].textContent" ","; } blocks.push({ id: blockid, deschtml: blockdescs, tags: tags, lasthappen: recenthappentime, blockcount: times, ucount: devices, }); return blocks; ----------------------日志导出管理器入口------------------------------ 延迟执行，否则找不到对应的按钮 let sleep="function" (time) new promise((resolve)> setTimeout(resolve, time));
    };
